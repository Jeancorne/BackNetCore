...
namespace ProyectoBack.Application.Helpers
{
    public class JWT
    {
        ...
        public string crearToken(clsUsuario usuario, List<string> claimsList)
        {
            var claims = new ClaimsIdentity();
            claims.AddClaim(new Claim("idUsuario", usuario.id.ToString()));
            claims.AddClaim(new Claim("usuario", usuario.usuario));
